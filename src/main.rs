mod image_manager;
mod pattern;
mod wfc;

use std::{ path::{ Path, PathBuf }, time::{ Duration, Instant } };

use crate::{ image_manager::image_io, wfc::{ model::WfcModel, solver::WfcSolver } };

use rand::{ rngs::StdRng, SeedableRng };

const PATTERN_SIZE: u32 = 2;

const OUTPUT_WAVE_WIDTH: usize = 64 * 5;
const OUTPUT_WAVE_HEIGHT: usize = 64 * 5;

const MAX_ATTEMPTS: usize = 100000;

// const SEED: u64 = 1203;

fn build_input_path() -> PathBuf {
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    Path::new(manifest_dir).join("assets/prismarine_bricks.png")
}

fn build_output_path() -> PathBuf {
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    Path::new(manifest_dir).join("assets/output.png")
}

fn print_duration(name: &str, duration: Duration) {
    println!("{:<30} {:>12.3} ms", name, duration.as_secs_f64() * 1000.0);
}

fn print_report_header() {
    println!("=== WFC PERFORMANCE REPORT ===");
    println!();
}

fn print_input_report(input_path: &Path, width: u32, height: u32, load_duration: Duration) {
    println!("Input:");
    println!("  Path: {:?}", input_path);
    println!("  Dimensions: {}x{}", width, height);
    println!();
    print_duration("Image loading", load_duration);
}

fn print_model_report(model: &WfcModel, creation_duration: Duration) {
    println!();
    println!("Model:");
    println!("  Pattern size: {}", model.get_pattern_size());
    println!("  Unique patterns: {}", model.pattern_count());
    println!("  Total frequency: {}", model.total_frequency());
    println!("  Adjacency rules: {}", model.get_rules().count_rules());
    println!();
    print_duration("Model creation", creation_duration);
}

fn print_wave_report() {
    println!();
    println!("Wave:");
    println!("  Width: {}", OUTPUT_WAVE_WIDTH);
    println!("  Height: {}", OUTPUT_WAVE_HEIGHT);
    println!("  Cells: {}", OUTPUT_WAVE_WIDTH * OUTPUT_WAVE_HEIGHT);
    println!("  Max attempts: {}", MAX_ATTEMPTS);
}

fn print_attempt_header(attempt: usize) {
    println!();
    println!("=== ATTEMPT {}/{} ===", attempt, MAX_ATTEMPTS);
}

fn print_solver_timings(solver: &WfcSolver) {
    let timings = solver.get_timings();
    println!("Observe total:     {:.3} ms", timings.observe.as_secs_f64() * 1000.0);
    println!("Propagate total:   {:.3} ms", timings.propagate.as_secs_f64() * 1000.0);
    println!("Observations:      {}", timings.observations);
    println!("Propagation calls: {}", timings.propagation_calls);
}

fn print_propagation_stats(solver: &WfcSolver) {
    let stats = solver.get_propagation_stats();

    println!("Propagation stats:");
    println!("  Queue pops:                  {}", stats.queue_pops);
    println!("  Removals processed:          {}", stats.removals_processed);
    println!("  Neighbor checks:             {}", stats.neighbor_checks);
    println!("  Allowed entries processed:   {}", stats.allowed_entries_processed);
    println!("  Unique affected patterns:    {}", stats.affected_patterns);
    println!("  Support checks:              {}", stats.support_checks);
    println!("  Removed patterns:            {}", stats.removed_patterns);
}

fn print_backtracking_stats(solver: &WfcSolver) {
    let stats = solver.get_backtracking_stats();

    println!("Backtracking stats:");
    println!("  Decisions:                   {}", stats.decisions);
    println!("  Backtracks:                  {}", stats.backtracks);
    println!("  Successful backtracks:       {}", stats.successful_backtracks);
    println!("  Rejected patterns:           {}", stats.rejected_patterns);
    println!("  Restored patterns:           {}", stats.restored_patterns);
    println!("  Committed decisions:         {}", stats.committed_decisions);
    println!("  History overflows:           {}", stats.history_overflows);
    println!("  Peak undo entries:           {}", stats.peak_undo_entries);
    println!("  Peak decision depth:         {}", stats.peak_decision_depth);
}

fn print_solver_report(
    solver: &WfcSolver,
    success: bool,
    solver_creation_duration: Duration,
    solve_duration: Duration
) {
    print_duration("Solver creation", solver_creation_duration);
    print_duration("Solve", solve_duration);
    println!("{:<30} {}", "Solve success", success);
    println!("{:<30} {}", "Fully collapsed", solver.get_wave().is_fully_collapsed());
    println!("{:<30} {}", "Has contradiction", solver.get_wave().has_contradiction());
    println!("{:<30} {}", "Unresolved cells", solver.get_wave().get_unresolved_count());
    println!("{:<30} {}", "Contradiction cells", solver.get_wave().get_contradiction_count());
    print_solver_timings(solver);
    print_propagation_stats(solver);
    print_backtracking_stats(solver);
}

fn print_attempt_restart(message: &str, attempt_duration: Duration) {
    println!("{}", message);
    print_duration("Attempt total", attempt_duration);
}

fn print_render_report(render_duration: Duration, width: u32, height: u32) {
    print_duration("Render", render_duration);
    println!("{:<30} {}x{}", "Output dimensions", width, height);
}

fn print_success_report(
    output_path: &Path,
    attempt: usize,
    image_load_duration: Duration,
    model_duration: Duration,
    total_solver_creation_duration: Duration,
    total_solve_duration: Duration,
    attempt_duration: Duration,
    generation_duration: Duration,
    program_duration: Duration
) {
    println!();
    println!("=== SUCCESS ===");
    println!("Output saved to: {:?}", output_path);
    println!();
    println!("Timings:");
    print_duration("Image loading", image_load_duration);
    print_duration("Model creation", model_duration);
    print_duration("Total solver creation", total_solver_creation_duration);
    print_duration("Total solve", total_solve_duration);
    print_duration("Successful attempt", attempt_duration);
    print_duration("Generation total", generation_duration);
    print_duration("Program total", program_duration);
    println!();
    println!("Attempts required: {}", attempt);
}

fn print_failure_report(
    total_solver_creation_duration: Duration,
    total_solve_duration: Duration,
    generation_duration: Duration,
    program_duration: Duration
) {
    println!();
    println!("=== FAILURE ===");
    println!("Generation failed after {} attempts.", MAX_ATTEMPTS);
    println!();
    print_duration("Total solver creation", total_solver_creation_duration);
    print_duration("Total solve", total_solve_duration);
    print_duration("Generation total", generation_duration);
    print_duration("Program total", program_duration);
}

fn main() {
    let program_start = Instant::now();
    print_report_header();
    let input_path = build_input_path();
    let image_load_start = Instant::now();
    let image = image_io::load_image(&input_path).unwrap();
    let image_load_duration = image_load_start.elapsed();
    print_input_report(&input_path, image.width(), image.height(), image_load_duration);
    let model_start = Instant::now();
    let model = WfcModel::from_image(&image, PATTERN_SIZE);
    let model_duration = model_start.elapsed();
    print_model_report(&model, model_duration);
    print_wave_report();
    let mut rng = StdRng::seed_from_u64(Instant::now().elapsed().as_nanos() as u64);
    let generation_start = Instant::now();
    let mut total_solve_duration = Duration::ZERO;
    let mut total_solver_creation_duration = Duration::ZERO;

    for attempt in 1..=MAX_ATTEMPTS {
        print_attempt_header(attempt);
        let attempt_start = Instant::now();
        let solver_creation_start = Instant::now();
        let mut solver = WfcSolver::new(OUTPUT_WAVE_WIDTH, OUTPUT_WAVE_HEIGHT, &model, &mut rng);
        let solver_creation_duration = solver_creation_start.elapsed();
        total_solver_creation_duration += solver_creation_duration;
        let solve_start = Instant::now();
        let success = solver.solve(&model, &mut rng);
        let solve_duration = solve_start.elapsed();
        total_solve_duration += solve_duration;
        print_solver_report(&solver, success, solver_creation_duration, solve_duration);

        if !success {
            print_attempt_restart(
                "Contradiction detected. Restarting generation...",
                attempt_start.elapsed()
            );
            continue;
        }

        let validation_start = Instant::now();
        let constraints_valid = solver.get_wave().validate_constraints(model.get_rules());
        let validation_duration = validation_start.elapsed();
        print_duration("Constraint validation", validation_duration);
        println!("{:<30} {}", "Constraints valid", constraints_valid);

        if !constraints_valid {
            print_attempt_restart(
                "Invalid final constraints. Restarting generation...",
                attempt_start.elapsed()
            );
            continue;
        }

        let render_start = Instant::now();
        let Some(output) = wfc::renderer::render_wave(
            solver.get_wave(),
            model.get_patterns()
        ) else {
            print_duration("Render", render_start.elapsed());
            println!("Cannot render the resolved wave. Restarting generation...");
            continue;
        };

        let render_duration = render_start.elapsed();
        print_render_report(render_duration, output.width(), output.height());

        let output_path = build_output_path();
        let save_start = Instant::now();
        image_io::save_image(&output, &output_path).unwrap();
        let save_duration = save_start.elapsed();
        print_duration("Image saving", save_duration);
        print_success_report(
            &output_path,
            attempt,
            image_load_duration,
            model_duration,
            total_solver_creation_duration,
            total_solve_duration,
            attempt_start.elapsed(),
            generation_start.elapsed(),
            program_start.elapsed()
        );

        return;
    }

    print_failure_report(
        total_solver_creation_duration,
        total_solve_duration,
        generation_start.elapsed(),
        program_start.elapsed()
    );
}
